...
void CDlgOrder::init(CClient2Dlg* dlg, CString dlgTitle, DialogType dlgType, CString managedAccounts)
{
	m_pDlg = dlg;
	m_dlgTitle = dlgTitle;
	m_dlgType = dlgType;
...
{
	CDlgSmartComboRoutingParams dlg(m_order->smartComboRoutingParams);
	if ( dlg.DoModal() == IDOK ) {
		// do nothing - params passed by ref
		// and is being updated inside dialog
	}
